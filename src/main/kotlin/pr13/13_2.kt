...
class Order(val customer: Customer, private val menu: Map<String, Dish>) {
    ...
    fun addDish(dishName: String, quantity: Int) {
        ...
        if (dish != null) { dishes[dish.name] = dishes.getOrDefault(dish.name, 0) + quantity }
    }
    fun getTotalPrice(): Double {
        var totalPrice = 0.0
        for ((dishName, quantity) in dishes) {
            val dish = menu[dishName]
            if (dish != null) { totalPrice += dish.price * quantity }
        }
        return totalPrice
    }
    override fun toString(): String {
        ...
        for ((dishName, quantity) in dishes) { sb.append("${dishName}: ${quantity}\n") }
...
class Restaurant {
    ...
    fun addDishToMenu(dish: Dish) { menu[dish.name] = dish }
    ...
    fun addDishToOrder(orderId: Int, dishName: String, quantity: Int) {
        ...
        if (order != null) { order.addDish(dishName, quantity) }
    }
    fun printMenu() {
        println("Меню:")
